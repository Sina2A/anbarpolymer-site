...
export default async function AdminGradesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <div className="admin-page-wrap" style={{ display: 'flex', gap: 24, padding: '32px 24px', fontFamily: "'Vazirmatn', Tahoma, sans-serif", direction: 'rtl', maxWidth: 1200, margin: '0 auto' }}>
      {/*
        واکنش‌گرایی (چیدمانِ عمودی روی موبایل) و نمایشِ ستونِ مشخصات فنی
        (دسکتاپ: کاملِ همه‌ی فیلدهای پرشده / موبایل: نشانِ خلاصهٔ «N مورد») حالا
        در globals.css سراسری با کلاس‌های .admin-page-wrap / .spec-full / .spec-badge
        تعریف شده — دیگه <style> تزریقی اینجا لازم نیست.
      */}
      <AdminNav currentUserName={currentUser.companyName} isAdmin={isAdmin} accessMap={myAccess || {}} activeSection="grades" />
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {canEdit && editingGrade && (
          <div style={{ ...cardStyle, borderColor: '#1e357b', borderWidth: 2 }}>
            ...
            <form action={updateGradeAction}>
              ...
              <div className="grade-form-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div className="grade-form-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 18, paddingTop: 16, borderTop: '1px dashed #d8dde3' }}>
              <div>
                <div style={mediaLabelStyle}>تصویر محصول <span style={{ fontWeight: 400, color: '#9199a3', fontSize: 11.5 }}>(اختیاری)</span></div>
              ...
              <div>
                <div style={mediaLabelStyle}>دیتاشیت فنی (PDF) <span style={{ fontWeight: 400, color: '#9199a3', fontSize: 11.5 }}>(اختیاری)</span></div>
        ...
        {canEdit && !editingGrade && (
          <div style={cardStyle}>
            ...
            <p style={{ fontSize: 11.5, color: '#9199a3', marginTop: 10, lineHeight: 1.9 }}>
              تصویر محصول و دیتاشیت هر دو اختیاری‌ان و محصول بدون اون‌ها هم ساخته می‌شه —
              هر وقت خواستی از طریق دکمه‌ی «ویرایش» آپلودشون کن.
            </p>
          </div>
        )}
        ...
        {/* ---------- جست‌وجو و فیلتر ---------- */}
        <form method="get" className="grades-filter-form" style={{ ...cardStyle, display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr auto', gap: 10, alignItems: 'center' }}>
        ...
        {/* ---------- جدول ---------- */}
        <div style={{ border: '1px solid #d8dde3', borderRadius: 12, overflowX: 'auto' }}>
    ...
    <tr style={{ borderTop: '1px solid #eceff3', opacity: g.isActive ? 1 : 0.5 }}>
      ...
      <td style={tdStyle}>
        {specKeys.length === 0 ? (
          ...
          <>
            {/* دسکتاپ: همه‌ی مشخصات پرشده به‌صورت کامل. موبایل: فقط تعداد. */}
            <div className="spec-full">
              {specKeys.map((k) => {
                const f = SPEC_FIELD_BY_KEY[k]
                return (
                  <div key={k} style={{ fontSize: 11, lineHeight: 1.8, whiteSpace: 'nowrap' }}>
                    <span style={{ color: '#6f7680' }}>{f?.label ?? k}:</span>{' '}
                    <strong style={{ fontFamily: 'monospace', color: '#1b1e24' }}>{specs[k]}</strong>
                    {f?.unit ? <span style={{ color: '#9199a3' }}> {f.unit}</span> : null}
                  </div>
                )
              })}
            </div>
